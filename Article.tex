...
The next most common category is those SWRs that were exclusively stationary (5149 of 23382 or 22\%). Unlike the stationary-continuous mixtures, most of these events represented a location close to the the animal's actual position (Figure \ref{5}C). There were, however, a number of stationary events  that represented positions far from the animal. We set a threshold of 30 cm distance from the animal to identify "non-local stationary trajectories" and found these trajectories in 1585 of 23382 (~7\%) of classified SWRs. About half of these (731 classified SWRs) were stationary throughout the duration of the SWR. These non-local stationary trajectories most commonly represented reward wells or choice points (Figure \ref{5}D), consistent with \cite{JaiDistincthippocampalcorticalmemory2017}. 

Finally, we examined the overall activity levels across dynamics and found that all the dynamics had similar average multiunit spiking rates by tetrode (Figure \ref{5}E). This indicates that all dynamics, including the slower dynamics such as stationary and stationary-continuous mixtures and importantly not just the continuous dynamics, were driven by sustained spiking information.
...
adjudicates/clarifies results of Pfiefer and Foster and Stella et al. Brownian motion but couldn't tell the speeds. Pfiefer and Foster didn't know the percentage of events that acted this way

Why our results are different from the Bayesian decoder
seek to describe as many SWR events, clusterless decoding, more permissive model with different dynamics, explicit descriptive model instead of implicit noisy model.

relationship to Johnson and Redish 2008, Maboudi, model represents middle ground between HMM and typical Bayesian decoder. allows SWR to quickly be identified because of the discrete states. Can work in 2D. Can precisely identify timing of dynamics because of acausal decoding

Can extend to incorporate other previously experienced environments. Add prior on dynamics of speed
...
To obtain the neural data used for decoding, hippocampus electrical potentials were recorded at 30 kHz, referenced to a corpus callosum tetrode, and then digitally filtered between 600 Hz and 6 kHz. Events were considered threshold crossing events if any one of the four tetrode wires exceeded a 60 mV threshold. These multiunit threshold crossing events and the associated peak amplitudes of the tetrodes wires---defined as the electrical potential value at the time of the maximum amplitude at the threshold crossing---were used as data for the clusterless version of the model.
...
Lastly, we evaluate the likelihood of the observations $p(O_{k} \mid x_{k}, I_{k})$ based on an encoding model fit during the encoding period. We assume the likelihood is the same for each dynamic $I_k$, so we only need to estimate $p(O_{k} \mid x_{k})$. It has been shown \cite{ZhangInterpretingNeuronalPopulation1998, BrownStatisticalParadigmNeural1998} that the likelihood with sorted spikes can be computed as:
$$
...
$$
where $N_{t_{k}}^{i}$ represents a spike at time $t_k$ from cell $i$, $\Delta_k$ is the time bin size, and $\lambda_{i}(t_{k} \mid x_{k})$ is the instantaneous firing rate of cell $i$ given position $x_{k}$. $\lambda_{i}(t_{k} \mid x_{k})$ is the "place field" of the cell, which can be estimated via generalized linear model during the encoding period (which we explain further in the Model Implementation section).
...
\prod^{E}_{i=1} [\lambda_{i}(t_{k}, \vec{m}_k \mid x_{k})\Delta_{k}]^{N_{t_{k}}^{i}} \\
    \exp[-\Lambda_{i}(t_{k} \mid x_{k})\Delta_{k}]
$$
where $\lambda_{i}(t_{k}, \vec{m}_k \mid x_{k})$ is now a generalized firing rate that depends on an associated wave form features $\Vec{m}$ and $\Lambda_{i}(t_{k} \mid x_{k})$ is a marginal firing rate that is equivalent to a place field estimated on multiunit spikes. Both of these quantities can be estimated via kernel density estimation based on multiunit spikes and their associated waveform features observed in the encoding period.

\subsection*{Model Implementation}
In order to implement the model, we used a grid based approximation of the latent position $x_k$ that respected the geometry of the track. For 1D linearized positions, we discretized the position space based on the same track graph we used for linearization by finding bins less than or equal to the chosen position bin size for each edge of the graph. For 2D positions, we discretized the position space by binning 2D positions occupied by the animal with equal sized bins of the chosen position bin size, followed by morphological opening to get rid of any holes smaller than the bin size. This was done using the scipy image processing tools \cite{SciPy1.0ContributorsSciPyfundamentalalgorithms2020}. This grid based approximation allows us to use Reimann sums to approximate the integrals in the causal filter and acausal smoother equations. We also used this grid in combination with the track graph in order to construct the appropriate 1D random walk transition matrices that respected the track geometry. To do this, we inserted the position bin centers as nodes in the track graph, and then computed the shortest path distance between all pairs of position bin centers. We then evaluated a zero mean Gaussian with a variance of 6 cm on these distances to get the appropriate transition matrix.
...
$$
where $\mu_i$ is the mean firing rate for tetrode $i$, $\pi(x)$ is the spatial occupancy of the animal on the track, and $p_{i}(x_k, \vec{m}_k)$ and $p_{i}(x_k)$ are the joint probability of observing waveform features $\Vec{m}$ and position $x$ and the probability of observing position $x$, respectively. $\pi(x)$, $p_{i}(x_k)$, and $p_{i}(x_k, \vec{m}_k)$ can all be estimated using the kernel density estimator:
$$
kde(y) = \frac{1}{N h_1...h_D} \sum^{N}_{i=1} \prod^{D}_{d=1} K_{d}\left(\frac{y_d - y_{i,d}}{h_d}\right)
$$
where $y$ is the data with $D$ dimensions, $K$ is a 1 dimensional Gaussian kernel with bandwidth $h_d$, $y_{i, d}$ is the data observed during the encoding period, $N$ is the number of observations in the encoding period.
\subsection*{Decoding}
