...
In order to validate the model's effectiveness in detecting short time scale changes in spiking dynamics, we first simulated 19 Poisson spiking cells with Gaussian place fields on a 180 cm virtual linear track. Each place field has a 36 cm variance and a 15 Hz peak firing rate, which is spaced every 10 cm along the virtual track. We then applied our decoding algorithm to the spiking sequence in Figure \ref{1}A. For the first third of this sequence, a single place cell fires repeatedly. For the middle third of the sequence, the cells fire in sequential spatial order. For the last third, the cells fire in a incoherent spatial pattern.

Decoding the spiking sequence requires specifying two elements: the data model and the dynamics model. For the data model, our decoder is the same as the Bayesian decoder. We compute an estimate of how each cell's firing rate varies over position (the place field, Figure \ref{1}B). This is used during decoding to compute the Poisson likelihood of position over time given the spiking sequence of interest. In contrast to the typical Bayesian decoder, we use small time bins (2 ms) because we are able to take advantage of the prior placed on the dynamics by the state space model. This allows us to detect changes on smaller time scales than would be possible with the 20 ms bin size used with the Bayesian decoder.

Next, we specify a dynamics model for how latent position can evolve at a given time (Figure \ref{1}C). We are interested in identifying three types of different dynamics---hover, continuous, and fragmented---which are modeled with 
...
\subsection*{Most replays are spatially coherent, but often not at the speed posited by canonical replay}
