...
We have previously shown [Asilomar REF] that the posterior $p(x_k, I_k \mid O_{1:T})$ can be directly estimated by applying the following recursive causal filter equation, starting with initial conditions $p(x_{0}, I_{0} \mid O_{0})$:
...
Lastly, we estimate the likelihood of the observations $p(O_{k} \mid x_{k}, I_{k})$. It has been shown [REFS, Zhang, Brown] that the likelihood with sorted spikes can be computed as:
$$
