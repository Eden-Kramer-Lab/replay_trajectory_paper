...
%DIF ADD /Users/edeno/Documents/GitHub/replay_trajectory_paper/Article.tex   Tue May  4 17:16:20 2021
...
\title{Hippocampal replay of experience at \DIFdelbegin \DIFdel{real-world }\DIFdelend \DIFaddbegin \DIFadd{slow }\DIFaddend speeds}
...
}\DIFaddend , but another report suggested that events that replay a single location are only seen in young animals \citep{StellaHippocampalReactivationRandom2019}. Thus, while it would seem useful to replay memories associated with single locations and perhaps even experiences occurring at real-world speeds, the prevalence of that type of replay in awake animals remains unclear.
...
%DIF >  Figure 2-Supplemental 3
\DIFaddbeginFL \figsupp[Spatial coverage of the track for each animal.]{}{\includegraphics[width=0.5\linewidth]{figures/Figure2-supplemental3/Figure2_v1-supplemental3}}\label{figsupp:fig2sup3}
...
\DIFdelbeginFL %DIFDELCMD < \figsupp[More examples of SWRs that would not be well-characterized by using the standard decoder]{A-I. More examples of SWRs that would not be well-characterized by using the standard decoder. Conventions are the same as in \FIG{3}.}{\includegraphics[width=1.0\linewidth]{figures/Figure3-supplemental1/Figure3_v4_supplemental1}}%%%
\DIFdelendFL \DIFaddbeginFL \figsupp[More examples of SWRs with non-constant speed trajectories]{A-I. More examples of SWRs that would not be well-characterized by using the standard decoder. Conventions are the same as in \FIG{3}.}{\includegraphics[width=1.0\linewidth]{figures/Figure3-supplemental1/Figure3_v4_supplemental1}}\DIFaddendFL \label{figsupp:fig3sup1}

% Figure 3-Supplemental 2
\DIFdelbeginFL %DIFDELCMD < \figsupp[Comparison of percentage of SWRs classified on real vs. position shuffled data]{Comparison of percentage of SWRs classified (that is, a SWR containing at least one of the five classifications) on real vs. position shuffled data for two epochs from different animals. Red line represents the percent of SWR events classified in that epoch for real data. The histogram represents the distribution after 50 shuffles of the position data. Position data was shuffled by resampling with replacement from the set of all observed positions in that epoch, destroying position information but preserving spiking timing and position occupancy.}{\includegraphics[width=0.8\linewidth]{figures/Figure3-supplemental2/Figure3_v2-supplemental2}}%%%
\DIFdelendFL \DIFaddbeginFL \figsupp[Percentage of SWRs classified decreases on position shuffled data]{Comparison of percentage of SWRs classified (that is, a SWR containing at least one of the five classifications) on real vs. position shuffled data for two recording sessions from different animals. Red line represents the percent of SWR events classified in that recording session for real data. The histogram represents the distribution after 50 shuffles of the position data. Position data was shuffled by resampling with replacement from the set of all observed positions in that recording session, destroying position information but preserving spiking timing and position occupancy.}{\includegraphics[width=0.8\linewidth]{figures/Figure3-supplemental2/Figure3_v2-supplemental2}}\DIFaddendFL \label{figsupp:fig3sup2}
\DIFaddbeginFL 

%DIF >  Figure 3-Supplemental 3
\figsupp[Percentage of spatially incoherent SWRs increases for run-swapped, circularly shuffled position data]{Comparison of percentage of spatially coherent and incoherent SWRs on real vs. run-swapped, circularly shuffled position. The red line represents the percent of spatially coherent or incoherent SWRs in that recording session for actual data. The histogram represents the distribution after 50 shuffles of the run from well to well as well as the circularly shuffled position (for each tetrode). This preserves local spatial correlations in the data but breaks the global spatial relationships between the spikes and the data.}{\includegraphics[width=0.8\linewidth]{figures/Figure3-supplemental3/Figure3_v1_supplemental3}}\label{figsupp:fig3sup3}
...
After testing our model on a single SWR event, we applied our clusterless decoding algorithm to hippocampal recordings from 10 rats performing the W-track spatial alternation task (\# tetrodes range: [10, 24], brain areas = [CA1, CA2, CA3]; some data previously used in \citep{KarlssonAwakereplayremote2009, KayConstantSubsecondCycling2020, CarrTransientSlowGamma2012}, position linearized to 1D). \DIFdelbegin \DIFdel{One major goal of our work is to }\DIFdelend \DIFaddbegin \DIFadd{We first confirmed that our cell population for each animal could closely track the position of the animal during behavior by comparing the most probable decoded position to the position of the animal (with the caveat that theta sequences and replay sequences will represent positions other than where the animal is and our resolution is limited by the size of our 3 cm position bins). We found, similar to other studies, that the average median difference is small (4 cm median difference, 3-6 cm 95\% CI, for all time in sessions, 5 fold cross validation). Importantly, we achieved this using small time bins (2 ms) compared to the commonly used 250 ms time bin that most studies use for decoding during run, demonstrating the power of our algorithm. We also verified that the population spiking for all positions on the track was consistent for each animal }\FIGSUPP[2]{fig2sup3}\DIFadd{.
}

\DIFadd{Having established the validity of our decoder in tracking the animal's position, we next wanted to }\DIFaddend assess the overall prevalence of spatial content across SWRs\DIFdelbegin \DIFdel{, so we detect }\DIFdelend \DIFaddbegin \DIFadd{. We detected }\DIFaddend SWRs using a permissive threshold (see Methods) to ensure that we include both the rare large amplitude events as well as the much more common small amplitude events. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend As expected, and providing further support for the validity of the model, we observe many replays that are classified as continuous throughout the entirety of the SWR, similar to those seen using the standard decoder (\FIGSUPP[2]{fig2sup2}A-F). However, we also observe many events with spatially coherent content that do not have this structure. For example, we observe trajectories that start in one direction and reverse back to the original position (\FIG{3}A, \FIGSUPP[3]{fig3sup1}B), trajectories that remain fixed in one position (\FIG{3}B, \FIGSUPP[3]{fig3sup1}G), and trajectories that jump between arms and between dynamics (\FIG{3}C, D, F, \FIGSUPP[3]{fig3sup1}F, H, I). We also observe SWRs that are spatially incoherent throughout the SWR (\FIGSUPP[3]{fig3sup1}A, D).

Using a 0.8 threshold, we are able to classify 89\% (23,\DIFdelbegin \DIFdel{382 of 26,160}\DIFdelend \DIFaddbegin \DIFadd{071 of 25,844}\DIFaddend ) of SWRs as containing at least one of the three dynamics or \DIFdelbegin \DIFdel{common }\DIFdelend dynamic mixtures. To ensure that this depends on the \DIFdelbegin \DIFdel{sequential }\DIFdelend firing of spatially tuned cells in the hippocampus, we resample position with replacement\DIFdelbegin \DIFdel{for two recording sessions, shuffling }\DIFdelend \DIFaddbegin \DIFadd{, which shuffles }\DIFaddend the relationship between spiking and \DIFdelbegin \DIFdel{spatial information and then fitting }\DIFdelend \DIFaddbegin \DIFadd{position, for two recording sessions, fit }\DIFaddend the encoding model \DIFdelbegin \DIFdel{. We }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend then decode the same SWR events containing the same spikes. Only 9\% of the SWRs are classified in the shuffled datasets, a value that is significantly less than that seen for the real data (p=0.02 for recording session 1, p=0.02 for recording session 2, \FIGSUPP[3]{fig3sup2}). \DIFaddbegin \DIFadd{This shows that our model will not classify dynamics when there is no spatial information present in the data.
}\DIFaddend 

Previous work focusing on spatially sequential replay reported that only a minority of events contain sequential spatial content \citep{KarlssonAwakereplayremote2009, FosterReversereplaybehavioural2006, DavidsonHippocampalReplayExtended2009}. We therefore asked what fraction of classified events contain spatially coherent content, which we define as SWRs containing any times with stationary, stationary-continuous mixture, or continuous dynamics (see Methods and \FIG{1}F). We find that 96\% (22,\DIFdelbegin \DIFdel{478 }\DIFdelend \DIFaddbegin \DIFadd{170 }\DIFaddend of 23,\DIFdelbegin \DIFdel{382}\DIFdelend \DIFaddbegin \DIFadd{071}\DIFaddend ) of classified SWRs and \DIFdelbegin \DIFdel{85}\DIFdelend \DIFaddbegin \DIFadd{86}\DIFaddend \% of all SWRs include spatially coherent structure and that this prevalence of spatially coherent structure is consistent across animals (\FIG{3}G). We then asked what fraction of events contained spatially incoherent content, defined as a SWR containing any times with fragmented or fragmented-continuous mixture dynamics. We find that only 14\% (3,\DIFdelbegin \DIFdel{353 }\DIFdelend \DIFaddbegin \DIFadd{295 }\DIFaddend of 23,\DIFdelbegin \DIFdel{382}\DIFdelend \DIFaddbegin \DIFadd{071}\DIFaddend ) of classified SWRs have time points with spatially incoherent structure (\FIG{3}G). \DIFaddbegin \DIFadd{To further validate this result, we performed a shuffle that randomized the order of the reward well to reward well runs and then circularly permuted the run. This, in contrast to our previous shuffle, should preserve correlation between spikes locally but destroy global spatial structure. Using this shuffle, we found that there was more shuffled spatially incoherent content compared to the real data and there was less spatially coherent content compared to the real data for two recording sessions (p=0.02 for recording session 1, p=0.02 for recording session 2, }\FIGSUPP[3]{fig3sup3}\DIFadd{).
}\DIFaddend 

To better compare our findings to previous work, we quantified the percentage of classified SWRs that contained continuous content, as would typically be analyzed when using the standard decoder. Here our findings were consistent with previous reports: in our datasets, 4,\DIFdelbegin \DIFdel{511 }\DIFdelend \DIFaddbegin \DIFadd{449 }\DIFaddend of 23,\DIFdelbegin \DIFdel{382 }\DIFdelend \DIFaddbegin \DIFadd{071 }\DIFaddend or 19\% of classified SWRs had time periods where the decoded position was classified as continuous (\FIG{3}G, 17\% of all SWRs). Thus, focusing on only high speed trajectories excludes a large fraction of events where there is evidence for spatially coherent, but not continuously changing, content. \DIFaddbegin \DIFadd{Note that here we did not limit our analysis to only those SWRs that had continuous dynamics for the entire SWR, as is assumed by the standard decoder in the case of fitting a line, because our decoder is much more flexible in terms of allowing different speeds during the SWR event.
}\DIFaddend 
...
\DIFdelendFL \DIFaddbeginFL \figsupp[Average 95\% HPD size increases on the 95\% highest posterior density on position shuffled data]{Comparison of average spatial coverage of the 95\% highest posterior density on real vs. position shuffled data for two recording sessions from different animals. Red line represents the average spatial position spanned by the highest posterior density on real data for all ripples. The histogram represents the distribution after 50 shuffles of the position data. Position data was shuffled by resampling with replacement from the set of all observed positions in that recording session, destroying position information but preserving spiking timing and position occupancy.}{\includegraphics[width=0.8\linewidth]{figures/Figure4-supplemental1/Figure4_v1-supplemental1}}\DIFaddendFL \label{figsupp:fig4sup1}
\DIFaddbeginFL 

%DIF >  Figure 4-Supplemental 2
\figsupp[Average 95\% HPD size increases on run-swapped, circularly shuffled position data]{Comparison of average spatial coverage of the 95\% highest posterior density on real vs. run-swapped, circularly shuffled position data for two recording sessions from different animals. Red line represents the average spatial position spanned by the highest posterior density on real data for all ripples. The histogram represents the distribution after 50 shuffles of the position data. The histogram represents the distribution after 50 shuffles of the run from well to well as well as the circularly shuffled position (for each tetrode). This preserves local spatial correlations in the data but breaks the global spatial relationships between the spikes and the data.}{\includegraphics[width=0.8\linewidth]{figures/Figure4-supplemental2/Figure4_v1-supplemental2}}\label{figsupp:fig4sup2}
...
\DIFaddbeginFL \figsupp[Distribution of dynamics is insensitive to parameters]{Conventions are the same as in \FIG{5}.}{\includegraphics[width=1.0\linewidth]{figures/Figure5-supplemental2/Figure5_v1_supplemental2}}\label{figsupp:fig5sup2}
...
\DIFadd{We performed several controls to see if our results were primarily influenced by the firing of interneurons, which typically do not project to other brain areas and may not be spatially specific in their firing (although note, interneuron firing still may have relevant position information and it is not clear that they should be excluded \mbox{%DIFAUXCMD
\citep{HangyaComplementaryspatialfiring2010, WilentDiscretePlaceFields2007}}\hspace{0pt}%DIFAUXCMD
). First, we checked if requiring five or more tetrodes to have spikes during the SWR (up from requiring two tetrodes be active) would influence the proportions of dynamics, with the thought that a single tetrode with a high firing interneuron might result in stationary or stationary-continuous mixture dynamics during SWRs. This had little effect on the overall proportions, with many stationary and stationary-continuous mixtures still present (}\FIGSUPP[5]{fig5sup2}\DIFadd{A). Indeed, most of our events included spikes from upwards of 10 tetrodes (}\FIGSUPP[5]{fig5sup3}\DIFadd{B), including within each dynamic (}\FIGSUPP[5]{fig5sup3}\DIFadd{A). Next, we tried removing spikes that had spike widths of less than 0.3 ms because interneurons typically have narrow spike widths. Note that this is a rather imprecise threshold, because typically this is computed on the average waveform for a putative cell, not on a single spike, and combined with a requirement for the putative cell to have a high firing rate, as interneurons typically exhibit high firing rates. Nevertheless, even with this imprecise criteria, we still observed a high proportion of SWRs with stationary and stationary-continuous mixture dynamics (}\FIGSUPP[5]{fig5sup2}\DIFadd{B). We also examined the 9 of 10 animals in our dataset that already had clustered cells, excluding putative interneurons based on spike width and firing rate and requiring that at least three putative pyramidal cells be active during the SWR. Although there were many fewer events to examine, we still observed many stationary and stationary-continuous mixtures in SWRs compared to continuous sequences (}\FIGSUPP[5]{fig5sup2}\DIFadd{D). Finally,  if interneurons were not spatially specific and driving the slower dynamics, we would expect to see the posterior be less spatially concentrated. Instead, we found that these dynamics tended to have narrow posteriors (as characterized by 95\% HPD region size in }\FIG{4}\DIFadd{E). This provides further evidence that our results were not the solely the result of the firing of interneurons. 
}

\DIFadd{Another possible concern is that many of these slow spatially coherent events occur when the animal is moving very slowly, meaning the stationary-continuous mixtures potentially reflect a slow theta sweep rather than a replay event. We do not believe this to be the case because the majority of our events occur when the animal is moving at speeds < 2 cm/s (}\FIGSUPP[5]{fig5sup3}\DIFadd{B) and when we restrict ourselves to SWRs where the animal was moving at speeds < 1 cm/s, we find similar proportion of SWRs with stationary and stationary-continuous mixture dynamics (}\FIGSUPP[5]{fig5sup2}\DIFadd{C).
}

\DIFadd{A third possible concern is that burst firing from a single cell is driving the stationary dynamics. There are several lines of evidence against this. First, as mentioned above, most of our SWRs had spikes from multiple tetrodes, including during stationary dynamics (}\FIGSUPP[5]{fig5sup3}\DIFadd{A), so it is unlikely that a single cell is driving these dynamics. Second, the stationary dynamics have a longer duration than one would expect from a burst alone (}\FIG{5}\DIFadd{B, 54 ms median duration, 38-74 ms quartiles vs. bursts which have a duration of 6-24 ms). Third, only ~50\% of the spikes had ISIs of less than 6 ms during the stationary, stationary-continuous, and continuous dynamics (}\FIGSUPP[5]{fig5sup3}\DIFadd{A), so if bursting occurs, it only occurs on half of the SWRs we have analyzed. It should be noted that this metric in particular should be interpreted with extreme caution, because with clusterless decoding, the ISI of the multiunit spikes is not the same as if we had an individual unit. The spikes could be from multiple neurons firing, which would not tell us if there was bursting or not. Nevertheless, even with these caveats, it seems unlikely that burst firing from cells is driving the stationary dynamics.
}

\DIFadd{One last concern is that we have combined spiking information from several hippocampal subfields (CA1, CA2, and CA3), which may influence the types of dynamics we observed. However, CA1 and CA3 spiking is largely coordinated within and across hemispheres during SWR replay \mbox{%DIFAUXCMD
\citep{CarrTransientSlowGamma2012} }\hspace{0pt}%DIFAUXCMD
and when we used only CA1 tetrodes to decode (}\FIGSUPP[5]{fig5sup2}\DIFadd{E), we get similar proportions of dynamics as when using all the tetrodes. Therefore, we feel our combination of spiking information from multiple subfields is appropriate.
}

%DIF >  Table 1
\begin{wraptable}{l}{0.5\textwidth}{
\begin{tabular}{lll}
\textbf{Dynamic}                       & \textbf{\begin{tabular}[c]{@{}l@{}}Percent.\\ Sig.\end{tabular}} & \textbf{Number Sig.}               \\ \hline
\multicolumn{1}{|l|}{Stat.}            & \multicolumn{1}{l|}{8\%}                                         & \multicolumn{1}{l|}{542 of 7176}   \\ \hline
\multicolumn{1}{|l|}{Stat.-Cont.-Mix.} & \multicolumn{1}{l|}{14\%}                                        & \multicolumn{1}{l|}{2253 of 16106} \\ \hline
\multicolumn{1}{|l|}{Cont.}            & \multicolumn{1}{l|}{30\%}                                        & \multicolumn{1}{l|}{1368 of 4511}  \\ \hline
\multicolumn{1}{|l|}{Cont.-Frag.-Mix.} & \multicolumn{1}{l|}{31\%}                                        & \multicolumn{1}{l|}{799 of 2562}   \\ \hline
\multicolumn{1}{|l|}{Frag.}            & \multicolumn{1}{l|}{15\%}                                        & \multicolumn{1}{l|}{244 of 1598}   \\ \hline
\end{tabular}
}
  \caption{\DIFadd{Number of significant SWRs that contain each dynamic. SWRs were fit using the random transform with 1000 shuffles of the posterior in each time bin.}}\label{tab:supp1}
\end{wraptable}

\DIFadd{Finally, an important question is how our method compares to the standard "Bayesian" decoder. This comparison is complicated due to the variety of parameters and inference methods used and referred to as the standard "Bayesian" decoder. We chose to use clusterless decoding in 20 ms time bins and three common methods of inferring the trajectory of the replay: the radon transform \mbox{%DIFAUXCMD
...
, and the MAP estimate (the maximum probability position of each time bin) \mbox{%DIFAUXCMD
...
. We then compared the speed of the replay for each dynamic and for the entire SWR (}\FIG{6}\DIFadd{). This reveals two key insights. First, the choice of method can give very different estimated speeds distributions for the SWRs. In particular, the methods that rely on fitting a line for the entire SWR (the radon transform and the linear regression), tend to estimate that the replays proceed at faster speeds (}\FIG{6}\DIFadd{A,C), whereas the MAP method (}\FIG{6}\DIFadd{B), which only relies on picking the most probable position for a given time bin and allows for more variable speeds, has a mixture of both fast and slow speeds. Second, the speeds from the MAP method are quite similar to the speeds estimated by our decoder (}\FIG{6}\DIFadd{D). This is to be expected because the MAP method is imposing the least amount of smoothing out of the other methods (only the 20 ms time bins) whereas the linear methods impose a larger amount of smoothing. Our decoder imposes more smoothing than the MAP estimate but much less than the line fitting methods and therefore is closer to the MAP estimates.
}
...
Previous work reported that less than half of SWRs or high-population-activity events contained replay \citep{KarlssonAwakereplayremote2009, FosterReversereplaybehavioural2006, DavidsonHippocampalReplayExtended2009}. This could lead one to the assumption that most events are not spatially coherent or meaningful. Our results indicate that this is not the case: in our dataset, the large majority of SWRs (85\%) contained spatially coherent content, as defined by some combination of stationary or continuous dynamics. These events \DIFdelbegin \DIFdel{included }\DIFdelend \DIFaddbegin \DIFadd{were mostly }\DIFaddend decoded representations where the most probable positions were highly spatially concentrated (i.e. had small HPD size)\DIFaddbegin \DIFadd{, indicating that they were driven by spatially specific firing}\DIFaddend . This was in \DIFdelbegin \DIFdel{stark }\DIFdelend contrast to unclassified events or events that included fragmented dynamics, which were associated with highly distributed decoded spatial representations.

Importantly, the spatially coherent events most often expressed a dynamic that was consistent with slower speeds of \DIFdelbegin \DIFdel{<}\DIFdelend \DIFaddbegin \DIFadd{less than }\DIFaddend 1 m/s \DIFaddbegin \DIFadd{(<100 cm/s), which is consistent with the speed that rats experience the environment}\DIFaddend . The next most common category were stationary events \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{JaiDistincthippocampalcorticalmemory2017, FarooqEmergencepreconfiguredplastic2019} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{YuDistincthippocampalcorticalmemory2017, FarooqEmergencepreconfiguredplastic2019} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend that activated an extended representation of a single location, while the "classical" replay events---continuous, extended trajectories through space---made up only about 19\% of classified events. \DIFdelbegin \DIFdel{Importantly, events }\DIFdelend \DIFaddbegin \DIFadd{Events }\DIFaddend with slower dynamics tended to persist for approximately the same length of time as events (\texttildelow50-100 ms) with faster continuous dynamics, consistent with a brief activation of a representation of a location or a short trajectory through space. Events with slower dynamics could also represent either the animal's current location or locations far from the animal, and similarly, previous work has shown that events with continuous dynamics most often begin at the animal's current location \citep{DavidsonHippocampalReplayExtended2009, KarlssonAwakereplayremote2009}.
...
We hypothesize that this large set of events supports the ability to rapidly retrieve representations of individual places and snippets of \DIFdelbegin \DIFdel{real-world }\DIFdelend experience. Previous findings demonstrated that SWRs with movement- or immobility-related activity engage specific patterns of spiking activity outside the hippocampus \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
}\DIFaddend . Thus, we would expect that the large set of events with slow dynamics would also engage specific patterns of activity outside the hippocampus, perhaps allowing animals to \DIFaddbegin \DIFadd{store and }\DIFaddend evaluate the value of specific locations or short movement trajectories without having to generate full trajectories \DIFdelbegin \DIFdel{to or through them\mbox{%DIFAUXCMD
...
\DIFadd{One possibility is that slow and fast dynamics are supported by different hippocampal subcircuits. \mbox{%DIFAUXCMD
...
showed that layer 2/3 MEC neurons increased their firing 50 ms before the start of longer duration SWRs (>100 ms). This raises the possibility that these slower replays are driven by CA3-CA1 interaction while the faster trajectories are more externally driven by MEC-CA1}\DIFaddend .

\DIFaddbegin \DIFadd{Our results showed that 19\% of SWRs contained continuous trajectories---meaning that portions of the SWR had spatially coherent trajectories that moved at high velocities---but only 4\% were continuous throughout the entire SWR. This, }\textit{\DIFadd{prima facie}}\DIFadd{, may seem at odds with previous results, which found between 5\% and 45\% of SWRs were "significant", meaning that they were well fit by a line compared to various shuffled versions of the dataset. Furthermore, these studies found trajectories on average move at much higher speeds (20x normal rat movement speed) than we report here. While the plurality of methods and parameters makes it hard to directly compare, we believe there are several reasons for this difference. First, our model is much more flexible because it allows the trajectory to change speed during the SWR, whereas fitting a line assumes a constant speed trajectory for the entire SWR. Many of the events that had continuous dynamics, and would be classified as having high speed trajectories, also exhibited stationary-continuous mixture dynamics (see for example }\FIG{1}\DIFadd{). Second, the line has to be fit to the whole SWR event, meaning that it is sensitive to the estimation of the start end of the SWR event. In contrast, our model makes an estimate for each time point and only depends on the previous and future time step, which minimizes the dependence of the event on those boundaries and any possible "noise" events within the SWR. This, along with our more permissive threshold for SWRs, allows us to characterize more SWRs than previous studies, rather than not analyze them.
 }

\DIFaddend Our results may also explain the seeming conflict between \cite{StellaHippocampalReactivationRandom2019}, who reported that hippocampal replay trajectories followed Brownian-like diffusion dynamics, and \cite{PfeifferAutoassociativedynamicsgeneration2015}, who reported that \DIFaddbegin \DIFadd{a subset of }\DIFaddend replay trajectories contained jumps in position. We found that while replay speeds are on average distributed log normally, there are still definitive "jumps" from one position to another in a small subset of the replays. Because our model makes it possible to identify different dynamics within the same SWR, we are able to identify both types of sequences.
...
\DIFaddend Fourth, our model \DIFdelbegin \DIFdel{avoids }\DIFdelend \DIFaddbegin \DIFadd{avoided using }\DIFaddend large temporal bins and does not make assumptions about the linear progression of events \DIFaddbegin \DIFadd{while still taking into account the uncertainty of the data}\DIFaddend . Instead, the model \DIFdelbegin \DIFdel{can use any size of bin and }\DIFdelend allows for movement in any direction \DIFdelbegin \DIFdel{. Finally, we used an acausal estimator }\DIFdelend \DIFaddbegin \DIFadd{allowed by geometry of the environment. Because the model gives a moment-by-moment estimate of the position and dynamic, this also reduces the effect of misidentifying the SWR start and end times. Finally, combined with an acausal estimation }\DIFaddend of latent position, \DIFdelbegin \DIFdel{which allowed }\DIFdelend \DIFaddbegin \DIFadd{this approach allows }\DIFaddend us to identify the specific timing of transitions in dynamics at a fine timescale.
...
\DIFadd{In order to calculate the HPD size, we first calculate the 95\% HPD region by determining the maximum threshold value $h$ that fulfills the follow equality:
}$$
\DIFadd{\int_{\{x: p(x_{k} \mid O_{1:T}) > h\}} p(x_{k} \mid O_{1:T})dx = 0.95
}$$
\DIFadd{The 95\% HPD region is the set of position bins with posterior values greater than the threshold $\{x : p(x_{k} \mid O_{1:T}) > h\}$. The HPD size is calculated by taking the integral of the members of this set:
}$$
...
\subsection*{\DIFadd{Shuffle Analysis of the Effect of Place Encoding on Classification of Dynamics and HPD Size}}
...
%DIFAUXCMD
\DIFdel{The 95\% highest posterior density (HPD) is a measurement of the spread of }\DIFdelend \DIFaddbegin \DIFadd{We also performed another shuffle that preserved more of the local correlation structure of spiking by shuffling the well-to-well runs and then circularly shuffling the position within the runs. A well-to-well run starts when the animal moves more than 5 cm from a well and ends when the animal moves back within 5 cm of a well. This mostly captures runs to separate wells (e.g. from the center well to }\DIFaddend the \DIFdelbegin \DIFdel{posterior probability and is defined as the region of the posterior that contains the top 95\% of values of the posterior probability \mbox{%DIFAUXCMD
\citep{CasellaStatisticalinference2001}}\hspace{0pt}%DIFAUXCMD
...
\DIFaddbegin \subsection*{\DIFadd{Standard "Bayesian" Decoding}}
...
\DIFadd{For the linear regression method, we draw 1000 position samples from each time bin according to the probability of position in that time bin. A linear regression is then fit to the sampled positions using time as a covariate. Fits were made separately for the posterior probabilities corresponding to the center arm with the left arm and the center arm with the right arm. Quality of the fit was assessed using the R-squared and the best fit between the arms was selected based on this metric.
}

\DIFadd{For the MAP estimate, we pick the maximum position bin for each time bin, which corresponds to the most probable position.
}
...
}\DIFaddend . All code is open-source and licensed under the MIT Software License. Classifier code can be easily installed as a python package with all requisite dependencies using pip or conda. See software repositories for specific details.
